...
<nav class="navbar navbar-dark navbar-expand-lg fixed-top bg-dark" id="mainNav">
        <div class="container"><a class="navbar-brand" href="#page-top">Book My Sports</a><button data-bs-toggle="collapse" data-bs-target="#navbarResponsive" class="navbar-toggler navbar-toggler-right" type="button" aria-controls="navbarResponsive" aria-expanded="false" aria-label="Toggle navigation"><i class="fa fa-bars"></i></button>
            <div class="collapse navbar-collapse" id="navbarResponsive">
                <ul class="navbar-nav ms-auto text-uppercase">
                    ...
                    <li class="nav-item"><a class="nav-link" href="achievements_display.php" style='color:#fed136;'>Achievements</a></li>
  ...
  if(isset($_POST['submit_search'])){
    ...
    if($rows==0){
        echo "<script type='text/javascript'>
        alert('Search failed!');
        window.location.href='achievements_display.php';
        </script>";
    }
    else{
        $count=0;
        while($x=mysqli_fetch_assoc($sq))
            {
                ...
                echo "
                    ...
                    <h5 class='text-white' style='font-family: 'Source Sans Pro', sans-serif;'>$ename</h5>
                      ...
                      <form method='post'>
                          <ul>
                              <li><button class='bg-info rounded-pill pt-1 pb-1 p-3' style='text-transform: capitalize;font-family: 'Source Sans Pro', sans-serif;' name='ach_like'>Like</button></li>
                          </ul>
                        </form></div>";
                        if(isset($_POST["ach_like"])){
                            $alikes+=1;
                            $tof=mysqli_query($con,"update `achievements` set ach_likes='$alikes' where ach_id='$aid'");
                            if($tof==true)
                                echo "<script type='text/javascript'>
